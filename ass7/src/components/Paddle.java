...
public class Paddle implements Sprite, Collidable {
    ...
    /**
     * Instantiates a new components.Paddle.
     *
    ...
    public void moveLeft() {
        ...
        if (keyboard.isPressed(KeyboardSensor.LEFT_KEY)
                && (this.shape.getUpperLeft().getX() > GameLevel.START_POINT + 15)) {
            if (this.shape.getUpperLeft().getX() - this.speed < GameLevel.START_POINT + 15) {
                this.shape = new Rectangle(new Point(GameLevel.START_POINT + 15,
                        this.shape.getUpperLeft().getY()),
                        this.shape.getWidth(), this.shape.getHeight());
            } else {
                this.shape = new Rectangle(
                        new Point(this.shape.getUpperLeft().getX() - this.speed, this.shape.getUpperLeft().getY()),
                        this.shape.getWidth(), this.shape.getHeight());
            }
        }
    }
    ...
    public void moveRight() {
        ...
        if (keyboard.isPressed(KeyboardSensor.RIGHT_KEY)
                && (this.shape.getUpperRight().getX() < GameLevel.WIDTH_SCR - 15)) {
            if (this.shape.getUpperRight().getX() + this.speed > GameLevel.WIDTH_SCR - 15) {
                this.shape = new Rectangle(
                        new Point(GameLevel.WIDTH_SCR - 15 - this.shape.getWidth(),
                                this.shape.getUpperLeft().getY()),
                        this.shape.getWidth(), this.shape.getHeight());
            } else {
                this.shape = new Rectangle(
    ...
    /**
     * set setSpeed to paddle.
     *
     * @param speed1 - the velocity
     */
